...
#### Stata 
The main script is `MAIN-global-edu-dashboard.do` which runs scripts under the following groups: <br>
...
1. Regression Outputs: (generated from `MAIN-md.R`).in `global-edu-dashboard/out/html` <br>
2. Histogram Report: `GLOBE_histograms.pdf`, found in `global-edu-dashboard/out` <br>
3. Sampling Analyses: `sampling.html` and `sch-asys-extend.html`, found in `global-edu-dashboard/out` <br>
4. By-Country In-Depth Descriptive Anlysis: each country file in `analysis-pt-a` folder, located at `global-edu-dashboard/out` <br>
5. Monte Carlo Analysis: `monte-carlo-results.xlsx`, in `global-edu-dashboard/out` <br>
6. Basic Descriptives and tables: `july-pres-tables.xlsx` in `global-edu-dashboard/out` <br>
7. July Joint EDU and BLAB presentation: located in `presentations` folder in `global-edu-dashboard/out` <br>


### Computer-readable Documents
The main working dataset is most easily accessible in Stata's .dta format, located at: `~/global-edu-dashboard/baseline/DataSets/final/merge_district_tdist.dta`.<br>
...
Full de-identified school and public official Stata-readable datasets can be found in this folder: `global-edu-dashboard/baseline/DataSets/Deidentified`
